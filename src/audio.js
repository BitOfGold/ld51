...
function _createReverb(length, decay) {
    let srate = ACX.sampleRate, bufferSize, convolver, i, noiseBuffer, outputL, outputR, _i
...
var zzfxP = (...t) => {let e=zzfxX.createBufferSource(),f=zzfxX.createBuffer(t.length,t[0].length,zzfxR);t.map((d,i)=>f.getChannelData(i).set(d)),e.buffer=f,e.connect(zzfxX.destination),e.start();return e}
...
window.zzfxMCreate = (data) => {

}

//plays asset
window.sound = (name, volume=1.0) => {
    let as = assets[name]
...
window.beep = (freq0=400, freq1=800, attack=0.01, sustain=0.1, decay=0.2, volume=1.0, type="sine") => {
    volume *= sfxvolume
...
function playNode(node, volume=1.0) {
    let upgain = ACX.createGain()
    upgain.gain.setValueAtTime(volume, ACX.currentTime)
    node.connect(upgain)
